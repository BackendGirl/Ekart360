            ...
            <div class="col-xl-4 col-md-4 col-sm-12">
                <div class="card statistial-visit">
                    ...
                    <div class="card-block">
                        ...
                        @if($key <= 9)
                        <p>
                        <mark style="color: {{ $latest_event->event_color }}">
                            <i class="fas fa-calendar-check"></i> {{ $latest_event->event_name }}
                        </mark>
                        <br>
                        <small>
                            @if(isset($setting->date_format))
                            {{ date($setting->date_format, strtotime($latest_event->event_start_date)) }}
                            @else
                            {{ date("Y-m-d", strtotime($latest_event->event_start_date)) }}
                            @endif

                            @if($latest_event->event_start_date != $latest_event->event_end_date)
                             <i class="fas fa-exchange-alt"></i> 
                            @if(isset($setting->date_format))
                            {{ date($setting->date_format, strtotime($latest_event->event_end_date)) }}
                            @else
                            {{ date("Y-m-d", strtotime($latest_event->event_end_date)) }}
                            @endif
                            @endif
                        </small>
                        </p>
